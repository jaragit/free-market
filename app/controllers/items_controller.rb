class ItemsController < ApplicationController
  ...
  def index
    ...
    ladies_items = Item.where(category: ladies_category.subtree).new_items
    mens_items = Item.where(category: mens_category.subtree).new_items
    kids_items = Item.where(category: kids_category.subtree).new_items
  ...
  def purchase
    ...
    customer_tokenn = current_user.card.customer_token
    Payjp::Charge.create(
      amount: @item.price, # 商品の値段
      customer: customer_tokenn, # 顧客、もしくはカードのトークン
      currency: 'jpy'  # 通貨の種類
    )
